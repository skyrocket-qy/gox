...
func TestMeasureTime(t *testing.T) {
	...
	if elapsed < sleepDuration.Nanoseconds() {
		t.Errorf("Expected elapsed time to be at least %d ns, got %d ns", sleepDuration.Nanoseconds(), elapsed)
	}
	...
	if elapsed > 2*sleepDuration.Nanoseconds() {
		t.Errorf("Elapsed time %d ns is much higher than expected %d ns", elapsed, sleepDuration.Nanoseconds())
	}
}
...
func TestProfileFunc(t *testing.T) {
	...
	tmpfile, err := os.CreateTemp("", "cpuprofile_*.prof")
	if err != nil {
	...
	result2, err := ProfileFunc(func() { time.Sleep(1 * time.Millisecond) }, true, tmpfile.Name(), 1)
	if err != nil {
