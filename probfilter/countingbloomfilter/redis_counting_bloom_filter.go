...
func Reserve(ctx context.Context, rdb *redis.Client, key string, errorRate float64, capacity int64) error {
	return rdb.Do(ctx, "BF.RESERVE", key, errorRate, capacity).Err()
}

// Add adds an item to the Counting Bloom filter.
// For a counting bloom filter, adding an item multiple times effectively increments its internal count.
...
// Note: RedisBloom's BF.DEL command is for Cuckoo Filters. For Bloom Filters, removal is not directly supported
// in a way that decrements counts. If true counting bloom filter behavior with decrements is needed,
// a different approach or data structure might be required (e.g., a custom implementation or a Count-Min Sketch).
// This function is included for completeness but its effectiveness for true counting bloom filter decrements is limited.
func Remove(ctx context.Context, rdb *redis.Client, key, value string) (bool, error) {
	...
	return false, fmt.Errorf("BF.DEL command is not supported for Bloom Filters. Use Cuckoo Filters for direct deletion.")
}
