...
func BloomFilterAdd(ctx context.Context, rdb *redis.Client, key string, value string) error {
	return rdb.Do(ctx, "BF.ADD", key, value).Err()
}

// BloomFilterExists checks if an item exists in the bloom filter
func BloomFilterExists(ctx context.Context, rdb *redis.Client, key string, value string) (bool, error) {
...
func BloomFilterReserve(ctx context.Context, rdb *redis.Client, key string, errorRate float64, capacity int64) error {
